...
function AddForm({ result }) {
  ...
  const [date, setdate] = useState(new Date(result.date));
  const [time, settime] = useState();
  const [place, setplace] = useState(result.place);
  const [distance, setdistance] = useState(Number(result.distance));
  const [people, setpeople] = useState(Number(result.people));
